...
Inclusions <- Inclusions %>%
   group_by(Author_Last, Condensed_Outcomes, Condensed_Discipline, level, Study_Design) %>% 
   summarize(total = n()) %>% 
...
Inclusions_2 <- Inclusions %>%
   group_by(Author_Last, Condensed_Outcomes, Condensed_Discipline, Study_Design) %>% 
   summarize(total = n()) %>% 
   mutate(proportion = total / sum(total))

table(Inclusions_2$level)
```
...
```{r check-inclusions}
Inclusions_check <-  Data %>% 
  filter(Include == "Y")

Inclusions_check <- Inclusions_check %>%
   group_by(Author_Last, Condensed_Outcomes, Condensed_Discipline, level, Study_Design) %>% 
   summarize(total = n()) %>% 
   mutate(proportion = total / sum(total))

table(Inclusions_check$level)
```
